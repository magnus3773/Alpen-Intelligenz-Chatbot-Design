...
(function() {
    ...
    widgetStyles.textContent = `
        ...
        /* Welcome (ausgeblendet, weil Registrierung entfernt) */
        ...
        /* ---- Dein zusätzliches CSS (eingebaut) ---- */
        body {
    ...
    // Create welcome screen with header (Welcome bleibt im DOM, ist aber ausgeblendet)
    const welcomeScreenHTML = `
        ...
        <div class="user-registration" style="display:none"></div>
    `;

    // Create chat interface (direkt aktiv)
    ...
    // Get DOM elements
    const startChatButton = chatWindow.querySelector('.chat-start-btn'); // existiert, aber hidden
    const chatBody = chatWindow.querySelector('.chat-body');
    const messagesContainer = chatWindow.querySelector('.chat-messages');
    const messageTextarea = chatWindow.querySelector('.chat-textarea');
    const sendButton = chatWindow.querySelector('.chat-submit');
    
    // Registration nodes existieren nicht mehr sichtbar – keine Events notwendig
    ...
    async function submitMessage(messageText) {
        ...
        // Falls keine Session existiert, jetzt erzeugen (minimale Ergänzung ohne Logikwechsel)
        if (!conversationId) conversationId = createSessionId();

        const email = ""; // keine Registrierung
        const name = "";  // keine Registrierung
